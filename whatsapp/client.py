...
_PLACEHOLDERS = {"YOUR_PHONE_NUMBER_ID", "YOUR_ACCESS_TOKEN", "YOUR_VERIFY_TOKEN", "YOUR_WEBHOOK_URL", "", None}
...
def send_interactive_message(*, phone_number_id: str, access_token: str, to: str, body_text: str) -> dict:
    if phone_number_id in _PLACEHOLDERS or access_token in _PLACEHOLDERS:
        return {
            "_simulated": True, "messaging_product": "whatsapp",
    ...
    for attempt in range(1, config.HTTP_MAX_RETRIES + 1):
        try:
            ...
            if 400 <= resp.status_code < 500:
                raise WhatsAppAPIError(f"Client error {resp.status_code}", resp.status_code, resp.text)
            last_exc = WhatsAppAPIError(f"Server error {resp.status_code}", resp.status_code, resp.text)
        except requests.RequestException as exc:
            last_exc = WhatsAppAPIError(f"Network error: {exc}")
        if attempt < config.HTTP_MAX_RETRIES: time.sleep(config.HTTP_RETRY_BACKOFF_SECONDS * attempt)
